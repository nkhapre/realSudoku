...
struct Graph{
    ...
    Waypoint<T>* bfs(Vertex<T>* start, Vertex<T>* end){
        ...
        HashTable<Vertex<T>*> seen(100, hashFunction);
        seen.add(start);
    ...
    private:
    static int hashFunction(Vertex<T>* t) {
        return reinterpret_cast<std::uintptr_t>(t) % 100;
    }
};
...
std::ostream& operator<<(std::ostream& os, const Graph<T>& graph){
    for (int i = 0; i < graph.vertices.size(); i++){
        Vertex<T>* currentVertex = graph.vertices[i];
        os << currentVertex->data << " Connected to --> ";
        
        for (int j = 0; j < currentVertex->neighbors.size(); j++){
            Neighbor<T>* currentNeighbor = currentVertex->neighbors[j];
            os << currentNeighbor->location->data;
            
            if (j < currentVertex->neighbors.size() - 1) {
                os << ", ";
            }
        }
